...
public class MaterialImportDAO {
            ...
            if (affectedRows > 0) {
                try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        materialImport.setId(generatedKeys.getInt(1));
                    }
                }
                return true;
            }
    ...
    */
    ...
    private void setStatementParameters(PreparedStatement stmt, MaterialImport materialImport) throws SQLException {
        stmt.setInt(1, materialImport.getMaterialId());
        stmt.setBigDecimal(2, materialImport.getImportQuantity());
        stmt.setBigDecimal(3, materialImport.getImportPrice());
        stmt.setBigDecimal(4, materialImport.getPricePerUnit());
        stmt.setString(5, materialImport.getSupplierName());
        stmt.setTimestamp(6, Timestamp.valueOf(materialImport.getImportDate()));
        stmt.setInt(7, materialImport.getCreatedBy());
        stmt.setString(8, materialImport.getNote());
    }
